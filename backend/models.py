...
# --- Storefront (PCStore) branding -----------------------------------------
# Singleton `store_branding` doc read publicly by PCStore via /api/store/config
# so brand changes in PCAdmin reflect on the storefront with no code changes.
_DEFAULT_STORE_BRANDING = {
    "_id": "singleton",
    "store_name": "PrettyCheap",                                   # bold header text on PCStore
    "tagline": "Pretty Prices · Cheap Deals · Every Day",          # subtitle under the name
    "logo": None,                                                   # data:image/... base64 or null
    "favicon": None,                                                # data:image/... base64 or null
    "tab_title": "PrettyCheap",                                     # browser tab <title>
}


class StoreBrandingUpdate(BaseModel):
    """PATCH body for `/api/store-branding`. All fields optional so partial
    saves don't clobber untouched fields. Images are data-URLs (base64) or
    `null` to clear, capped ~3 MB each."""
    store_name: Optional[str] = Field(default=None, min_length=1, max_length=80)
    tagline: Optional[str] = Field(default=None, max_length=160)
    tab_title: Optional[str] = Field(default=None, max_length=80)
    logo: Optional[str] = Field(default=None, max_length=4_500_000)
    favicon: Optional[str] = Field(default=None, max_length=4_500_000)
...
class OrderCreate(BaseModel):
    # Single-product path (kept for backward compat). Optional so a multi-line
    # `items` array can be sent instead for a whole-cart checkout.
...
# Singleton schedule for permanently removing category documents with no products.
# `next_run_at` is stored as a timezone-aware UTC ISO timestamp.
CATEGORY_CLEANUP_SCHEDULE_DEFAULTS = {
    "id": "singleton",
    "enabled": False,
    "next_run_at": None,
...
class CategoryCleanupScheduleUpdate(BaseModel):
...
_DEFAULT_EMAIL_TEMPLATES = {
    ...
    # returning the raw link in the response/log so testing isn't blocked.
    "portal_base_url": "",
    "verification": {
        "subject": "Verify your email to activate your account",
        "heading": "Confirm your email address",
        "body": "Hi {name}, thanks for creating an account. Please confirm your email address to activate your account and start shopping.",
...
# Currently powers the storefront "Customer Support" menu — specifically the
# "Get Help" link shown in PCStore's customer-account dropdown. Admins point
# it at either an external URL or one of the store's static pages.
# ---------------------------------------------------------------------------
SITE_PAGES = [
...
SITE_PAGE_SLUGS = [p["slug"] for p in SITE_PAGES]

SITE_MENUS_DEFAULTS: dict = {
    "id": "singleton",
    "get_help": {
        "enabled": True,
        "label": "Get Help",
        "link_type": "page",   # "url" | "page"
        "url": "",
        "page": "faq",
    },
    "faq_items": [
    ...
    "footer": {"enabled": True, "links": [], "custom_text": ""},
}


class GetHelpLinkBody(BaseModel):
    enabled: Optional[bool] = None
    label: Optional[str] = None
    link_type: Optional[str] = None  # "url" | "page"
...
class FooterLinkBody(BaseModel):
    id: Optional[str] = Field(default=None, max_length=120)
    label: str = Field(min_length=1, max_length=120)
    link_type: Literal["url", "page"] = "page"
    url: Optional[str] = Field(default="", max_length=2000)
    page: Optional[str] = Field(default="home", max_length=120)


class FooterConfigBody(BaseModel):
    enabled: Optional[bool] = None
    links: Optional[List[FooterLinkBody]] = Field(default=None, max_length=50)
    custom_text: Optional[str] = Field(default=None, max_length=5000)
...
class SiteMenusUpdate(BaseModel):
    ...
    footer: Optional[FooterConfigBody] = None


# ---------------------------------------------------------------------------
# Admin Accounts (separate from customer-portal auth). Managed inside
# Admin Settings › Accounts. Two roles today:
#   - "admin"   : full access
#   - "manager" : Orders, Customers, Products only (no Store Mgmt / Payments / Settings)
# The main admin is seeded once on startup and cannot be deleted.
# ---------------------------------------------------------------------------
...
class AdminAccountUpdate(BaseModel):
    ...
    password: Optional[str] = None   # optional on update — omit to keep the current hash
...
# Seeded on first startup so the platform always has at least one signed-in
...
# ---------------------------------------------------------------------------
# Singleton document (id="singleton") stored under `country_access_settings`.
# `allowed_country_codes` is the current allow-list — every request whose
# Cloudflare `CF-IPCountry` header isn't in this set is blocked with a
# minimal 403 response.
#
# `bypass_token` is a random URL-safe token embedded in the emergency URL:
#   GET /api/security/bypass/<token>
# Visiting it grants 24 hours of access to the calling IP regardless of
# their country. The token itself is stored in the DB and can be rotated
# from the Security tab (`POST /api/security/country-access/regenerate-token`).

# By default: Australia + Morocco are allowed (per initial requirement).
_DEFAULT_COUNTRY_ACCESS = {
    "_id": "singleton",
    "allowed_country_codes": ["AU", "MA"],
}

# 24 hour bypass session TTL in seconds.
BYPASS_SESSION_TTL_SECONDS = 24 * 60 * 60


class CountryAccessUpdate(BaseModel):
    """PATCH body for `/api/security/country-access`.

    `allowed_country_codes` — list of ISO 3166-1 alpha-2 codes that are
    currently allowed. Any code not on this list is blocked. The empty
    list is a valid state (blocks everyone) but is deliberately allowed
    so the admin can lock themselves in and rely solely on the bypass URL.
    """
