"""ISO 3166-1 alpha-2 country list used by the Country Access Control feature.

Kept in its own module because it's large and static — importing it into
`models.py` inflates that file for no reason.
"""

# (code, name). Deliberately ordered alphabetically by common English name
# for a nice UI list. Includes the 249 codes currently on the ISO list plus
# the informal "Kosovo" (XK) since Cloudflare's `CF-IPCountry` header uses
# it. Kept in Python for zero-round-trip access on the backend; the frontend
# ships its own copy for the toggle list.
COUNTRIES = [
    ...
    ("SX", "Sint Maarten (Dutch part)"), ("SK", "Slovakia"),
